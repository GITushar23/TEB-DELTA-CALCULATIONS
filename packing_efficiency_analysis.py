...
def write_summary_markdown(
    ...
    if len(errors):
        lines.extend([
            ...
            "## Notes",
            "",
            "- Some configurations could not be parsed or matched; see `packing_parse_errors.csv`.",
        ])
...
def analyze_dataset(spec: DatasetSpec) -> None:
    ...
    metrics_df.to_csv(tables_dir / "configuration_packing_metrics.csv", index=False)
    merged.to_csv(tables_dir / "packing_vs_delta_merged.csv", index=False)
    summary_df.to_csv(tables_dir / "chemsys_summary.csv", index=False)
    errors_df.to_csv(tables_dir / "packing_parse_errors.csv", index=False)
