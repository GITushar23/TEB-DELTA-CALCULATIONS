...
-------------------------------------------------------------------------------
Reads aggregated *_output.csv files produced by run.py (parallel_ternary.py).
Those files live in:
    mp_pipeline_results/aggregated/Sc-Ti-O_output.csv   (example)

Identical dip-detection logic to delta_calc.py (binary), just handles the
...
def main():
    ...
    if not ternary_files:
        print("[ERROR] No ternary files found. "
              "Make sure run.py has completed and produced M1-M2-L_output.csv files.")
        return
